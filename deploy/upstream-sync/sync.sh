...
# Always verify fork hooks after merge attempt (even if conflicted, files may be partial).
if [[ -x "$(dirname "$0")/check-fork-hooks.sh" ]] || [[ -f "$(dirname "$0")/check-fork-hooks.sh" ]]; then
  echo
  echo "== verifying fork hooks =="
  bash "$(dirname "$0")/check-fork-hooks.sh" || {
    echo "hook check failed — restore P1/P3/P4/P5 call sites per docs/FORK_HOOKS.md before tagging" >&2
    exit 1
  }
fi
