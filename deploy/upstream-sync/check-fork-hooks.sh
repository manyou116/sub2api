...
need_rg "webimg multipart image_N aliases" \
  'strings.HasPrefix(name, "image_")' \
  backend/internal/service/openai_images.go
need_rg "webimg multipart bracket aliases" \
  'strings.HasPrefix(name, "image[")' \
  backend/internal/service/openai_images.go
need_rg "webimg bulk default supports large accounts" \
  "BulkMaxAccounts = 5000" \
  backend/internal/service/openai_web_images_service.go
need_rg "webimg bulk uses configured concurrency" \
  "BulkProbeConcurrency" \
  backend/internal/service/openai_web_images_service.go
need_rg "webimg inflight-full helper" \
  "IsInflightFull" \
  backend/internal/service/openai_web_images_service.go
need_rg "webimg scheduler inflight-full gate" \
  "isWebImageInflightFullForRequest" \
  backend/internal/service/openai_account_scheduler_webimg.go
need_rg "hot scheduler filters webimg inflight saturation" \
  "isWebImageInflightFullForRequest" \
  backend/internal/service/openai_account_scheduler.go
need_rg "frontend chunks large webimg bulk selections" \
  "const chunkSize = 200" \
...
need_rg "admin update preserves webimg extra" \
  "preserve openai_web_images" \
  backend/internal/service/admin_account.go
need_rg "wire injects webimg into capacity service" \
  "SetWebImages" \
  backend/cmd/server/wire_gen.go
need_rg "wire injects webimg admin service" \
  "SetOpenAIWebImagesService" \
  backend/cmd/server/wire_gen.go
...
need_rg "group platform accepts kiro" \
  "oneof=anthropic openai gemini antigravity grok kimi zhipu deepseek kiro composite" \
...
need_rg "kiro estimated cache usage hook" \
  "applyKiroEstimatedCacheUsage" \
  backend/internal/service/kiro_chat_service.go
...
need_rg "frontend kiro preset mappings" \
  "kiroPresetMappings" \
  frontend/src/composables/useModelWhitelist.ts
...
need_file deploy/docker-compose.swarm.yml
need_file deploy/docker-compose.swarm-local.yml
need_file deploy/Caddyfile.swarm-local
need_file deploy/SWARM_DEPLOY.md
need_file backend/cmd/server/cleanup.go
need_file backend/cmd/server/cleanup_test.go
need_rg "server marks health draining on shutdown" \
  "routes.MarkShuttingDown()" \
  backend/cmd/server/main.go
need_rg "server shutdown timeout config field" \
  "ShutdownTimeout" \
  backend/internal/config/config.go
need_rg "server shutdown drain delay config field" \
  "ShutdownDrainDelay" \
  backend/internal/config/config.go
need_rg "server shutdown timeout default" \
  "server.shutdown_timeout" \
  backend/internal/config/config.go
need_rg "cleanup parallel honors deadline" \
  "RunCleanupParallel" \
  backend/cmd/server/wire_gen.go
need_rg "cleanup sequential honors deadline" \
  "RunCleanupSequential" \
  backend/cmd/server/wire_gen.go
need_rg "compose stop grace for in-flight requests" \
  "stop_grace_period: 320s" \
  deploy/docker-compose.yml
need_rg "swarm stop grace for in-flight requests" \
  "stop_grace_period: 320s" \
  deploy/docker-compose.swarm.yml
need_rg "swarm dnsrr app service" \
  "endpoint_mode: dnsrr" \
  deploy/docker-compose.swarm.yml
need_rg "swarm start-first rolling update" \
  "order: start-first" \
  deploy/docker-compose.swarm.yml
need_rg "swarm drain delay env" \
  "SERVER_SHUTDOWN_DRAIN_DELAY" \
  deploy/docker-compose.swarm.yml
need_rg "swarm caddy routes to task DNS" \
  "tasks.sub2api:8080" \
  deploy/Caddyfile.swarm-local
