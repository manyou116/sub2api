...
const webRequireDownloadAttachment = computed(() => {
  if (props.webImagesStatus) return props.webImagesStatus.require_download_attachment === true
  return webExtra.value?.require_download_attachment === true
})

const webImagesSuffix = computed(() => {
  if (!webEnabled.value) return ''
  if (webIsRateLimited.value) return t('admin.accounts.webImages.rateLimitedShort')
  if (webRequireDownloadAttachment.value) return t('admin.accounts.webImages.originalAttachmentShort')
...
const webImagesTooltip = computed(() => {
  ...
  const originalInfo = webRequireDownloadAttachment.value
    ? ` | ${t('admin.accounts.webImages.requireDownloadAttachment')}`
    : ''
  if (reason) return `${base}${resolved}${limitInfo}${originalInfo} (${reason})`
  return `${base}${resolved}${limitInfo}${originalInfo}`
})
