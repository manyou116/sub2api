<template>
  <div v-if="visible" class="min-w-[140px] space-y-1">
    <div class="flex flex-wrap items-center gap-1">
      ...
      <span
        v-if="enabled && requireDownloadAttachment"
        class="inline-flex items-center rounded-full bg-cyan-100 px-2 py-0.5 text-[11px] font-medium text-cyan-800 dark:bg-cyan-900/40 dark:text-cyan-200"
        :title="t('admin.accounts.webImages.requireDownloadAttachmentHint')"
      >
        {{ t('admin.accounts.webImages.originalAttachmentShort') }}
      </span>
    </div>
...
const requireDownloadAttachment = computed(() => {
  if (status.value) return status.value.require_download_attachment === true
  return extraCfg.value?.require_download_attachment === true
})
