      ...
      >
        <div class="py-1">
          <template v-if="account">
            ...
            <button v-if="isOpenAIOAuth" @click="$emit('refresh-image-quota', account); $emit('close')" class="flex w-full items-center gap-2 px-4 py-2 text-sm text-pink-600 hover:bg-gray-100 dark:hover:bg-dark-700">
              <Icon name="refresh" size="sm" />
              {{ t('admin.accounts.refreshImageQuota') }}
            </button>
...
const emit = defineEmits(['close', 'test', 'stats', 'schedule', 'reauth', 'refresh-token', 'refresh-image-quota', 'recover-state', 'reset-quota', 'set-privacy'])
const { t } = useI18n()
