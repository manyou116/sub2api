...
func TestPlatformMigrationsPreserveExistingKiroQuota(t *testing.T) {
	...
	upgrade, err = migrations.FS.ReadFile("238_opencode_go_platform.sql")
	require.NoError(t, err)
	for range 2 {
		_, err = tx.ExecContext(ctx, string(upgrade))
		require.NoError(t, err, "OpenCode upgrade must retain Kiro and MiniMax rows")
	}
	_, err = tx.ExecContext(ctx, `INSERT INTO user_platform_quotas
		(user_id, platform, daily_limit_usd) VALUES (1, 'opencode_go', 7)`)
	require.NoError(t, err)

	purge, err := migrations.FS.ReadFile("238_purge_unlimited_user_platform_quotas.sql")
	require.NoError(t, err)
	_, err = tx.ExecContext(ctx, string(purge))
	require.NoError(t, err)
	...
	var configured int
	err = tx.QueryRowContext(ctx, `SELECT count(*) FROM user_platform_quotas
		WHERE platform IN ('kiro', 'minimax', 'opencode_go') AND deleted_at IS NULL`).Scan(&configured)
	require.NoError(t, err)
	require.Equal(t, 3, configured)
}
