...
// OpenAIWebImagesConfig controls the optional ChatGPT Web image path (fork feature).
// Default: disabled. Enable via gateway.openai_web_images.enabled and per-account extra.
...
func setDefaults() {
	...
	// OpenAI Web Images (fork): default OFF so upstream behavior is unchanged.
	viper.SetDefault("gateway.openai_web_images.enabled", false)
	...
	viper.SetDefault("gateway.openai_web_images.probe_on_schedule", false)
	viper.SetDefault("gateway.openai_web_images.unknown_quota_policy", "strict")
