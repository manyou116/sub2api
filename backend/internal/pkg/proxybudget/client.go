...
func (c *Client) call(ctx context.Context, path string, input, output any) error {
	...
	for attempt := 0; attempt < 2; attempt++ {
		req, err := http.NewRequestWithContext(ctx, http.MethodPost, endpoint.String(), bytes.NewReader(payload)) //nolint:gosec // G704: operator-configured budget URL, validated by New; paths are internal constants.
		if err != nil {
			return err
		}
		req.Header.Set("Content-Type", "application/json")
		req.Header.Set("X-Proxy-Budget-Token", c.token)
		resp, err := c.http.Do(req) //nolint:gosec // G704: destination comes from trusted server configuration, never an inbound request.
		if err != nil {
