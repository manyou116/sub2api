...
func TestAcquireImageSlotForSelection_WaitPlanReleaseClearsInflight(t *testing.T) {
	const accountID int64 = 987654321
	account := &Account{ID: accountID, Platform: PlatformOpenAI, Type: AccountTypeOAuth}
	req := OpenAIAccountScheduleRequest{RequiredImageCapability: OpenAIImagesCapabilityNative}
	origReleaseCount := 0
	selection := &AccountSelectionResult{
		Account:  account,
		WaitPlan: &AccountWaitPlan{AccountID: accountID, MaxConcurrency: 1},
		ReleaseFunc: func() {
			origReleaseCount++
		},
	}

	if got := legacyImagesInflightCount(accountID); got != 0 {
		t.Fatalf("initial inflight = %d, want 0", got)
	}
	if !acquireImageSlotForSelection(req, account, selection) {
		t.Fatal("expected image slot acquisition to succeed")
	}
	if got := legacyImagesInflightCount(accountID); got != 1 {
		t.Fatalf("inflight after acquire = %d, want 1", got)
	}
	if acquireImageSlotForSelection(req, account, &AccountSelectionResult{}) {
		t.Fatal("expected second image slot acquisition to fail")
	}
	...
	if origReleaseCount != 1 {
		t.Fatalf("original release count = %d, want 1", origReleaseCount)
	}
}
