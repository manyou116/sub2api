...
6. Optional original-file mode: account `extra.openai_web_images.require_download_attachment=true`
   appends a downloadable PNG/original-file instruction using the request `size`
   and waits for matching attachment file candidates instead of returning the
   first compressed chat preview pointer.
...
Per account (admin UI/PATCH or `extra` JSON):
...
```json
{
  "openai_web_images": {
    ...
    "thinking_effort": "min",
    "require_download_attachment": true
  }
}
```
