...
func (s *OpenAIGatewayService) forwardOpenAIImagesLegacyWeb(
	...
	// left brand-new accounts stuck in quota_unknown under strict policy.
	...
	if st != nil && !st.Schedulable {
		// quota_unknown/no_quota/cooldown are account-capacity issues, not upstream 429 storms.
		// Use 503 so clients/ops see "temporarily unavailable" instead of generic rate-limit text.
		status := http.StatusServiceUnavailable
		reason := st.UnschedulableReason
		if reason == "cooldown" || reason == "inflight_full" || reason == "no_quota" {
			status = http.StatusTooManyRequests
		}
		return nil, &UpstreamFailoverError{
			StatusCode:             status,
			ResponseBody:           []byte(`{"error":{"message":"web image account not schedulable: ` + reason + `"}}`),
	...
	if cfg.RequireDownloadAttachment {
		prompt = appendOpenAIWebImagesDownloadAttachmentPrompt(prompt, parsed.Size)
	}
	genReq := webdriver.GenerateRequest{
		Prompt: prompt, Model: sel.UpstreamModel, ThinkingEffort: sel.ThinkingEffort,
...
// UsesOpenAIWebImagesPath reports whether this account will serve images via ChatGPT Web.
func (s *OpenAIGatewayService) UsesOpenAIWebImagesPath(account *Account) bool {
	return s.shouldUseOpenAILegacyWebImages(account)
}

func appendOpenAIWebImagesDownloadAttachmentPrompt(prompt, size string) string {
