...
func TestOpenAIWebImages_ParseAndShouldUse(t *testing.T) {
	...
	require.False(t, cfg.RequireDownloadAttachment)
	require.Equal(t, int64(2), cfg.Stats.Success)
	require.True(t, svc.ShouldUseWebPath(acc))
	acc.Extra["openai_web_images"] = map[string]any{"enabled": true, "require_download_attachment": true}
	cfg = svc.ParseAccountConfig(acc)
	require.True(t, cfg.RequireDownloadAttachment)
...
func (r *webImgAccountRepo) ListModelAvailabilityCandidates(context.Context, *int64, []string, bool) ([]Account, error) {
	return nil, nil
}
...
func TestOpenAIWebImages_MarkSuccessPreservesRequireDownloadAttachment(t *testing.T) {
	repo := &webImgAccountRepo{accounts: map[int64]*Account{
		1: {ID: 1, Platform: PlatformOpenAI, Type: AccountTypeOAuth, Extra: map[string]any{
			"openai_web_images": map[string]any{"enabled": true, "require_download_attachment": true},
		}},
	}}
	svc := NewOpenAIWebImagesService(webImgTestCfg("memory"), nil, repo)
	...
	require.NoError(t, err)
	require.True(t, st.RequireDownloadAttachment)
}

func TestOpenAIWebImages_PatchRequireDownloadAttachment(t *testing.T) {
	repo := &webImgAccountRepo{accounts: map[int64]*Account{
		1: {ID: 1, Platform: PlatformOpenAI, Type: AccountTypeOAuth, Extra: map[string]any{
			"openai_web_images": map[string]any{"enabled": true},
		}},
	...
	on := true
	require.NoError(t, svc.PatchAccount(context.Background(), 1, OpenAIWebImagesBulkPatch{RequireDownloadAttachment: &on}))
	require.True(t, svc.ParseAccountConfig(repo.accounts[1]).RequireDownloadAttachment)

	off := false
	require.NoError(t, svc.PatchAccount(context.Background(), 1, OpenAIWebImagesBulkPatch{RequireDownloadAttachment: &off}))
	require.False(t, svc.ParseAccountConfig(repo.accounts[1]).RequireDownloadAttachment)
}

func TestOpenAIWebImages_BulkPatchRejectsInvalidBatchesAsBadRequest(t *testing.T) {
	cfg := webImgTestCfg("memory")
	cfg.Gateway.OpenAIWebImages.BulkMaxAccounts = 2
	svc := NewOpenAIWebImagesService(cfg, nil, &webImgAccountRepo{accounts: map[int64]*Account{}})

	_, err := svc.BulkPatch(context.Background(), nil, OpenAIWebImagesBulkPatch{})
	require.Error(t, err)
	require.True(t, infraerrors.IsBadRequest(err))

	_, err = svc.BulkPatch(context.Background(), []int64{1, 2, 3}, OpenAIWebImagesBulkPatch{})
	require.Error(t, err)
	require.True(t, infraerrors.IsBadRequest(err))
}
...
func TestOpenAIWebImages_ResolveUpstream(t *testing.T) {
	...
	fixedAcc := &Account{ID: 2, Platform: PlatformOpenAI, Type: AccountTypeOAuth, Credentials: map[string]any{"plan_type": "team"}, Extra: map[string]any{
		"openai_web_images": map[string]any{"enabled": true, "model_mode": "fixed", "model": "gpt-5.6-luna-wm", "thinking_effort": "min"},
	}}
	sel = svc.ResolveUpstream(fixedAcc)
	require.Equal(t, "fixed", sel.ModelMode)
	require.Equal(t, "gpt-5.6-luna-wm", sel.UpstreamModel)
	require.Equal(t, "min", sel.ThinkingEffort)
...
func TestOpenAIWebImages_PatchInheritOmitsEnabled(t *testing.T) {
	...
	raw, ok := repo.accounts[1].Extra["openai_web_images"].(map[string]any)
	require.True(t, ok)
