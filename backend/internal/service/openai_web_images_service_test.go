...
func webImgTestCfg(enabled bool, backend string) *config.Config {
	return &config.Config{Gateway: config.GatewayConfig{OpenAIWebImages: config.OpenAIWebImagesConfig{
		Enabled: enabled, DefaultMaxInflight: 2, QuotaCacheTTLSeconds: 300, UnknownQuotaPolicy: "strict",
...
func TestOpenAIWebImages_ParseAndShouldUse(t *testing.T) {
	svc := NewOpenAIWebImagesService(webImgTestCfg(true, "memory"), nil, nil)
	...
	require.False(t, NewOpenAIWebImagesService(webImgTestCfg(false, "memory"), nil, nil).ShouldUseWebPath(acc))
}
...
func TestOpenAIWebImages_RedisInflight(t *testing.T) {
	...
	svc := NewOpenAIWebImagesService(webImgTestCfg(true, "redis"), rdb, nil)
...
func TestOpenAIWebImages_BulkPatchAndStats(t *testing.T) {
	...
	svc := NewOpenAIWebImagesService(webImgTestCfg(true, "memory"), nil, repo)
	enabled := true
	maxInf := 2
...
func TestOpenAIWebImages_ClearCooldownRemovesRedisKeys(t *testing.T) {
	...
	svc.cfg = &config.Config{Gateway: config.GatewayConfig{OpenAIWebImages: config.OpenAIWebImagesConfig{
		Enabled: true, InflightBackend: "redis", RedisKeyPrefix: "sub2api:webimg:", RateLimitCooldownSeconds: 600,
	}}}
