...
func TestListAccountsAllowingTextRateLimitIncludesRLAccount(t *testing.T) {
	...
	svc := &OpenAIGatewayService{
		accountRepo: repo,
		webImages:   NewOpenAIWebImagesService(&config.Config{Gateway: config.GatewayConfig{OpenAIWebImages: config.OpenAIWebImagesConfig{InflightBackend: "memory"}}}, nil, nil),
		cfg:         &config.Config{},
	}
	gid := int64(2)
