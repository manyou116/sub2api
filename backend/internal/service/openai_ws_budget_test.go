...
type budgetReaderLoopConn struct {
	*openAIWSReaderLoopFakeConn
	forceCloses atomic.Int64
}

func (*budgetReaderLoopConn) UpstreamPingCount() int64 { return 7 }

func (c *budgetReaderLoopConn) CloseNow() error {
	c.forceCloses.Add(1)
	return c.openAIWSReaderLoopFakeConn.Close()
}

func TestBudgetingOpenAIWSRetainsPoolReaderLoopAndForceClose(t *testing.T) {
	var settles atomic.Int64
	...
	require.NoError(t, err)
	inner := &budgetReaderLoopConn{openAIWSReaderLoopFakeConn: newOpenAIWSReaderLoopFakeConn()}
	wrapped := &budgetingOpenAIWSClientConn{next: inner, lease: lease}
	conn := newOpenAIWSConn("budget-reader-loop", 1, wrapped, nil)
	defer conn.close()

	require.True(t, conn.hasReaderLoop())
	require.Eventually(t, func() bool { return inner.readers.Load() == 1 }, time.Second, 5*time.Millisecond)
	require.NoError(t, conn.pingWithTimeout(time.Second))
	require.Equal(t, int64(7), conn.upstreamPingCount())
	conn.abort()
	require.Equal(t, int64(1), inner.forceCloses.Load())
	require.Eventually(t, func() bool { return inner.readers.Load() == 0 }, time.Second, 5*time.Millisecond)
	require.Eventually(t, func() bool { return settles.Load() == 1 }, time.Second, 5*time.Millisecond)
}
