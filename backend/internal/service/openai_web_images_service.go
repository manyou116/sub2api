...
type OpenAIWebImagesAccountConfig struct {
	...
	RequireDownloadAttachment bool                 `json:"require_download_attachment,omitempty"`
...
type OpenAIWebImagesStatus struct {
	...
	RequireDownloadAttachment bool                 `json:"require_download_attachment"`
...
func (s *OpenAIWebImagesService) GetStatus(ctx context.Context, account *Account) (*OpenAIWebImagesStatus, error) {
	...
	st := &OpenAIWebImagesStatus{
		...
		RequireDownloadAttachment: cfg.RequireDownloadAttachment,
...
func (s *OpenAIWebImagesService) PatchAccount(ctx context.Context, accountID int64, patch OpenAIWebImagesBulkPatch) error {
	...
	if patch.RequireDownloadAttachment != nil {
		cfg.RequireDownloadAttachment = *patch.RequireDownloadAttachment
	}
...
func (s *OpenAIWebImagesService) saveAccountConfig(ctx context.Context, accountID int64, account *Account, cfg OpenAIWebImagesAccountConfig) error {
	payload := map[string]any{
		...
		"require_download_attachment": cfg.RequireDownloadAttachment,
...
func (s *OpenAIWebImagesService) BulkPatch(ctx context.Context, accountIDs []int64, patch OpenAIWebImagesBulkPatch) (*OpenAIWebImagesBulkResult, error) {
	c := s.cfgOrDefault()
	if len(accountIDs) == 0 {
		return nil, infraerrors.BadRequest("WEBIMG_BULK_EMPTY", "account_ids required")
	}
	if len(accountIDs) > c.BulkMaxAccounts {
		return nil, infraerrors.BadRequest(
			"WEBIMG_BULK_TOO_MANY",
	...
	results := make(chan itemResult, len(accountIDs))
		...
		go func() {
			...
			for id := range jobs {
				if err := ctx.Err(); err != nil {
					results <- itemResult{id: id, err: err}
					continue
				}
				results <- itemResult{id: id, err: s.PatchAccount(ctx, id, patch)}
			}
		}()
	}
	...
	for item := range results {
		if item.err != nil {
			result.Failed++
			result.Errors = append(result.Errors, OpenAIWebImagesBulkError{AccountID: item.id, Error: item.err.Error()})
			continue
		}
		result.Updated++
	}
...
func (s *OpenAIWebImagesService) StartBulkProbe(ctx context.Context, accountIDs []int64) (*OpenAIWebImagesBulkJob, error) {
	...
	if len(accountIDs) > c.BulkMaxAccounts {
		return nil, infraerrors.BadRequest("WEBIMG_BULK_TOO_MANY", fmt.Sprintf("too many accounts: %d (max %d)", len(accountIDs), c.BulkMaxAccounts))
	}
