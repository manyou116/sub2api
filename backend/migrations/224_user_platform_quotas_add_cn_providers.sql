...
-- Fork note: keep kiro in the same CHECK when replayed with P5.
--
-- 背景：国产供应商进入 AllowedQuotaPlatforms（internal/service/domain_constants.go），
-- 注册时 GetDefaultPlatformQuotas 会为全部 9 平台预填充默认配额行，但 157 号迁移的
...
--
-- 修复：把约束与代码平台列表（PlatformKimi/PlatformZhipu/PlatformDeepseek + fork Kiro）对齐。
